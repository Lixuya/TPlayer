...
public final class AudioTrack {
	...
	private AudioEngine audioEngine;   //andy
	private long audioEngineInst = 0;  //andy
	private int outputChannelCount = 0;
...
//		}
	...
	public void configure(String mimeType, int channelCount, int sampleRate, int pcmEncoding,
		...
		Log.i("AudioEngine", "---pcmFrameSize---" + pcmFrameSize + "---channelConfig---" + channelConfig);
	...
	public int initialize(int sessionId) throws InitializationException {
		...
		audioEngineInst = audioEngine.audioInit(1, 512, 4, sampleRate, false);
		audioEngine.audioSet(audioEngineInst, false, 0, false, true, 1.0f);
	...
	public int handleBuffer(ByteBuffer buffer, int offset, int size, long presentationTimeUs)
		...
		//andy add start
		int audioframesize = 512;
		float[] audioInput = new float[audioframesize * 4];
		float[] audioOutput = new float[audioframesize * 2];
		short[] audioOutput2 = new short[audioframesize * outputChannelCount];
		float[] metadata = new float[12];
		int tmp32;
		//andy add end
		...
		if (true) { //andy
			...
			if (bytesToWrite > 0) {
				...
				//andy hack start
				if (bytesToWrite >= audioframesize * outputChannelCount * 2) {
					bytesToWrite = audioframesize * outputChannelCount * 2;

					if (outputChannelCount == 4 || outputChannelCount == 6) {
						byte[] data_8 = new byte[bytesToWrite];
						for (int i = 0; i < bytesToWrite; i++) {
							data_8[i] = temporaryBuffer[i + temporaryBufferOffset];
						}
						//Log.i("AudioEngine","---bytesToWrite---"+bytesToWrite);
						ByteBuffer fulldata = ByteBuffer.allocate(bytesToWrite);
						fulldata.put(data_8);
						fulldata.order(ByteOrder.LITTLE_ENDIAN);

						short[] sounddata = new short[bytesToWrite / 2];
						for (int i = 0; i < bytesToWrite / 2; i++) {
							sounddata[i] = fulldata.getShort(i * 2);
						}
						//bytesWritten=audioTrack.write(sounddata, 0,bytesToWrite/2);
						//bytesWritten=bytesWritten*2;

						if (outputChannelCount == 4) {
							for (int j = 0; j < audioframesize; j++) {
								audioInput[j * 4] = (float) sounddata[j * outputChannelCount] / 32768;
								audioInput[j * 4 + 1] = (float) sounddata[j * outputChannelCount + 1] / 32768;
								audioInput[j * 4 + 2] = (float) sounddata[j * outputChannelCount + 2] / 32768;
								audioInput[j * 4 + 3] = (float) sounddata[j * outputChannelCount + 3] / 32768;
							}
						...
						for (int k = 0; k < audioframesize; k++) {
							for (int j = 0; j < 2; j++) {
								tmp32 = (int) (audioOutput[k * 2 + j] * 32768);
								if (tmp32 > 32767) tmp32 = 32767;
								else if (tmp32 < -32768) tmp32 = -32768;
								audioOutput2[k * outputChannelCount + j] = (short) tmp32;
							}
						}
						bytesWritten = audioTrack.write(audioOutput2, 0, audioframesize * outputChannelCount);
						bytesWritten *= 2;
					} else {
						bytesWritten = audioTrack.write(temporaryBuffer, temporaryBufferOffset, bytesToWrite);
					}
	...
	public void reset() {
		if (isInitialized()) {
			...
			audioEngine.audioRelease(audioEngineInst);//andy
			new Thread() {
				@Override
				public void run() {
					try {
